...
import { Globe, XCircle, Upload, Copy, ExternalLink, Loader2, Shield, RefreshCw, Info, Receipt, ChevronDown, CheckCircle, Folder, Globe2, File, FileText, Image, Code, FolderOpen } from 'lucide-react';
...
export default function DeploySitePanel() {
  ...
  const { deployFolder, deploying, deployProgress, deployResults, errors, reset: resetDeploy } = useFolderUpload();
  const { 
    checkUploadStatus, 
  ...
  const calculateTotalCost = (): number => {
    ...
    const totalWinc = (totalSize / (1024 ** 3)) * Number(wincForOneGiB);
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (result.manifestId) {
        setDeployMessage({ 
          type: 'success', 
          text: `Site deployed successfully! Manifest: ${result.manifestId}` 
        });
      }
    } catch (error) {
        ...
        {selectedFolder && selectedFolder.length > 0 && (
          <div className="mt-6">
            <div className="bg-surface rounded-lg p-4">
              ...
              <div className="border-t border-default/30 pt-4">
                <button
                ...
                >
                  <div className="flex items-center gap-2">
                    <FolderOpen className="w-4 h-4 text-turbo-red" />
                    <span className="text-sm font-medium text-fg-muted">View File Structure</span>
                  </div>
                            ...
                            <div className={folderPath !== 'root' ? 'ml-4 space-y-0.5' : 'space-y-0.5'}>
                              {structure[folderPath]
                                ...
                                .map(({ file, path }, index) => {
                                  ...
                                  return (
                                    <div key={index} className="flex items-center justify-between text-link hover:text-fg-muted transition-colors py-1">
                                      <div className="flex items-center gap-2 flex-1 min-w-0">
                                        <FileIcon className="w-3 h-3 text-link flex-shrink-0" />
                                        <span className="truncate">{fileName}</span>
                                      </div>
                                      <span className="text-link/70 text-xs ml-2 flex-shrink-0">
                                        {fileSize}
                                      </span>
                                    </div>
                                  );
